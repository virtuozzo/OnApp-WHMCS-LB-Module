$( document ).ready( function () {
	...
	// Create Cpus Element
	var cpus_label = ONAPP_LANG[ 'onapplbcpus' ];
	...
	// Create Cpus Element
	var cpu_shares_label = ONAPP_LANG[ 'onapplbcpuguarantee' ];
	...
	// Create Port Speed Element
	var rate_limit_label = ONAPP_LANG[ 'onapplbratelimit' ];
	...
	secondTable.append( row_title( ONAPP_LANG[ 'onapplbclusterconfiguration' ], 2 ) );
	secondTable.append( row_html( port_label, port_html ) );
	secondTable.append( row_title( ONAPP_LANG[ 'onapplbinstance' ], 2 ) );
	...
	secondTable.append( row_title( ONAPP_LANG[ 'onapplbtype' ], 2 ) );
	...
	// AutoScale Staff
	var outCpuUsageLabels = [ ONAPP_LANG[ 'onapplbifcpuusage' ], ONAPP_LANG[ 'onapplbisabove' ], '%', ONAPP_LANG[ 'onapplbfor' ], ONAPP_LANG[ 'onapplbminutes' ], ONAPP_LANG[ 'onapplbadd' ], ONAPP_LANG[ 'onapplbmorevms' ] ];
	...
	var outFreeRamLabels = [ ONAPP_LANG[ 'onapplbiffreeram' ], ONAPP_LANG[ 'onapplbislessthen' ], 'MB', ONAPP_LANG[ 'onapplbfor' ], ONAPP_LANG[ 'onapplbminutes' ], ONAPP_LANG[ 'onapplbadd' ], ONAPP_LANG[ 'onapplbmorevms' ] ];
	...
	var inCpuUsageLabels = [ ONAPP_LANG[ 'onapplbifcpuusage' ], ONAPP_LANG[ 'onapplbislessthen' ], '%', ONAPP_LANG[ 'onapplbfor' ], ONAPP_LANG[ 'onapplbminutes' ], ONAPP_LANG[ 'onapplbremove' ], ONAPP_LANG[ 'onapplbmorevms' ] ];
	...
	var inFreeRamLabels = [ ONAPP_LANG[ 'onapplbiffreeram' ], ONAPP_LANG[ 'onapplbmorethen' ], 'MB', ONAPP_LANG[ 'onapplbfor' ], ONAPP_LANG[ 'onapplbminutes' ], ONAPP_LANG[ 'onapplbremove' ], ONAPP_LANG[ 'onapplbmorevms' ] ];
	...
	thirdTable.append( row_title( ONAPP_LANG[ 'onapplbclusternodetemplates' ], 2 ) );
	...
	thirdTable.append( row_title( ONAPP_LANG[ 'onapplbclusternodeparameters' ], 2 ) );
	...
	forthTable.append( row_title( ONAPP_LANG[ 'onapplbautoscaleoutparams' ], 8 ) );
	...
	forthTable.append( row_title( ONAPP_LANG[ 'onapplbautoscaleinparams' ], 8 ) );
	...
	hvZonesSelect.children().each( function () {
		if( in_array( $( this ).val(), hvZonesRel ) === false && $( this ).val() != 0 && hvZones[$( this ).val()] != 'autoselect' ) {
	...
	// checkboxes
	if( outCpuEnabled ) {
		$( 'input[name="lb[auto_scaling_out_cpu_attributes][enabled]"]' ).attr( 'checked', 'checked' ).change();
	}
	if( inCpuEnabled ) {
		$( 'input[name="lb[auto_scaling_in_cpu_attributes][enabled]"]' ).attr( 'checked', 'checked' ).change();
	}
	if( outMemoryEnabled ) {
		$( 'input[name="lb[auto_scaling_out_memory_attributes][enabled]"]' ).attr( 'checked', 'checked' ).change();
	}
	if( inMemoryEnabled ) {
		$( 'input[name="lb[auto_scaling_in_memory_attributes][enabled]"]' ).attr( 'checked', 'checked' ).change();
	}
	// inputs and selects
	if( outCpuValue ) {
		$( 'input[name="lb[auto_scaling_out_cpu_attributes][value]"]' ).val( outCpuValue );
	}
	if( outCpuMinutes ) {
		$( 'select[name="lb[auto_scaling_out_cpu_attributes][for_minutes]"]' ).val( outCpuMinutes );
	}
	if( outCpuUnits ) {
		$( 'input[name="lb[auto_scaling_out_cpu_attributes][units]"]' ).val( outCpuUnits );
	}
	if( outMemoryValue ) {
		$( 'input[name="lb[auto_scaling_out_memory_attributes][value]"]' ).val( outMemoryValue );
	}
	if( outMemoryMinutes ) {
		$( 'select[name="lb[auto_scaling_out_memory_attributes][for_minutes]"]' ).val( outMemoryMinutes );
	}
	if( outMemoryUnits ) {
		$( 'input[name="lb[auto_scaling_out_memory_attributes][units]"]' ).val( outMemoryUnits );
	}
	if( inCpuValue ) {
		$( 'input[name="lb[auto_scaling_in_cpu_attributes][value]"]' ).val( inCpuValue );
	}
	if( inCpuMinutes ) {
		$( 'select[name="lb[auto_scaling_in_cpu_attributes][for_minutes]"]' ).val( inCpuMinutes );
	}
	if( inCpuUnits ) {
		$( 'input[name="lb[auto_scaling_in_cpu_attributes][units]"]' ).val( inCpuUnits );
	}
	if( inMemoryValue ) {
		$( 'input[name="lb[auto_scaling_in_memory_attributes][value]"]' ).val( inMemoryValue );
	}
	if( inMemoryMinutes ) {
		$( 'select[name="lb[auto_scaling_in_memory_attributes]][for_minutes]"]' ).val( inMemoryMinutes );
	}
	if( inMemoryUnits ) {
		$( 'input[name="lb[auto_scaling_in_memory_attributes][units]"]' ).val( inMemoryUnits );
	}
...
function create_slider_html( input_html, max, min, step, name ) {
	return '<div class="input-with-slider">' + input_html + '    <div class="slider" style="float:left; margin:5px 15px 0 5px; width:200px;" max="' + max + '" min="' + min + '" step="' + step + '" target="' + name + '" width="200"></div></div>';
}
...
function prepare_config_json() {
	...
	var html = '<input type="hidden" value="' + configurations + '" name="packageconfigoption[2]"/>';
